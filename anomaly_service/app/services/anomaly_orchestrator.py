    ...
    try:
        ...
        if "is_anomaly" in df.columns:
            ...
            df["anomaly_reasons"] = df.apply(
                lambda r: result_df["anomaly_reasons"].iloc[r.name] if r["is_anomaly"] else "none",
                axis=1
            )
        else:
        ...
        # Safe cast in case is_anomaly is stored as string 'True'/'False' in CSV
        is_anomaly_bool = df["is_anomaly"].apply(
            lambda x: str(x).strip().lower() == "true" if isinstance(x, str) else bool(x)
        )
        run.anomaly_count = int(is_anomaly_bool.sum())
        run.avg_anomaly_score = float(df["anomaly_score"].astype(float).mean())
...
def insert_anomaly_records(
    ...
    for idx, row in df.iterrows():
        ...
        record = AnomalyRecord(
            ...
            is_anomaly=bool(str(row["is_anomaly"]).strip().lower() == "true"
